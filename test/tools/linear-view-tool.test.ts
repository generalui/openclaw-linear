...
describe("linear_view tool", () => {
  describe("list", () => {
    it("returns all custom views", async () => {
      ...
      const data = parse(
        await createViewTool().execute("call-1", { action: "list" }),
      );
  ...
  describe("get", () => {
    ...
    it("returns error when viewId is missing", async () => {
      const data = parse(
        await createViewTool().execute("call-1", { action: "get" }),
      );
  ...
  describe("create", () => {
    ...
    it("returns error when name is missing", async () => {
      const data = parse(
        await createViewTool().execute("call-1", { action: "create" }),
      );
  ...
  describe("delete", () => {
    ...
    it("returns error when viewId is missing", async () => {
      const data = parse(
        await createViewTool().execute("call-1", { action: "delete" }),
      );
      expect(data.error).toContain("viewId is required");
    });
  });
