...
describe('formatConsolidatedMessage', () => {
  ...
  it('formats multiple assigned actions as a numbered list', () => {
    const actions = [
      ...
      makeAction({
        detail: 'Assigned to issue ENG-43: Update API docs',
        issueId: 'issue-43',
        issueLabel: 'ENG-43: Update API docs',
      }),
    ]
  ...
  it('handles mixed event types', () => {
    const actions = [
      ...
      makeAction({
        ...
        detail: 'Unassigned from issue ENG-50: Old task',
        issueLabel: 'ENG-50: Old task',
      }),
      makeAction({
        event: 'issue.reassigned',
        detail: 'Reassigned away from issue ENG-51: Moved task',
        issueLabel: 'ENG-51: Moved task',
      }),
    ]
