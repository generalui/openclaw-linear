...
const REMOVAL_EVENTS = new Set(['issue.unassigned', 'issue.reassigned', 'issue.removed', 'issue.state_removed'])
...
export class InboxQueue {
  ...
  async enqueue(entries: EnqueueEntry[]): Promise<number> {
    ...
    try {
      ...
      const removalIds = new Set(entries.filter((e) => REMOVAL_EVENTS.has(e.event)).map((e) => e.id))

      // Handle priority updates — update matching items' priority in-place
      const priorityUpdates = new Map(
        entries.filter((e) => e.event === 'issue.priority_changed').map((e) => [e.id, mapPriority(e.issuePriority)]),
      )
      ...
      if (removalIds.size > 0) {
        filtered = existing.filter((item) => !(removalIds.has(item.issueId) && item.event === 'ticket'))
      ...
      const existingKeys = new Set(filtered.map((item) => `${item.id}:${item.event}`))
